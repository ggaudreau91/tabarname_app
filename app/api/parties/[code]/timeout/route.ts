...
// POST /api/parties/[code]/timeout — le temps du tour est écoulé sans placement:
// on clôt le tour comme raté (all_wrong). Idempotent côté DB (forfeit_turn
// ne fait rien si le tour est déjà résolu).
export async function POST(
  ...
  const { data, error } = await svc.rpc("forfeit_turn", { p_turn_id: body.turn_id });
